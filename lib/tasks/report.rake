...
namespace :report do
  ...
  task :pmc => :environment do
    ...
    next if Source.active.where(name: "pmc").empty?

    csv = Report.usage_csv(source: "pmc", format: ENV['FORMAT'], month: ENV['MONTH'], year: ENV['YEAR'])
  ...
  task :counter => :environment do
    ...
    # check that source is active
    next if Source.active.where(name: "counter").empty?

    csv = Report.usage_csv(source: "counter", format: ENV['FORMAT'], month: ENV['MONTH'], year: ENV['YEAR'])
  ...
  task :all_stats => [:environment, :alm_stats, :pmc_html_stats, :pmc_pdf_stats, :pmc_combined_stats, :pmc_stats, :counter_html_stats, :counter_pdf_stats, :counter_xml_stats, :counter_combined_stats, :counter_stats, :combined_stats, :alm_private_stats, :combined_private_stats, :zip]
end
