...
describe ReportMailer, :type => :mailer do
  describe "error report" do
    ...
    it "provides a link to the admin dashboard" do
      ...
      expect(body_html).to have_link('Go to admin dashboard', href: "http://#{ENV['SERVERNAME']}/notifications")
    end
  end
  ...
  describe "status report" do
    ...
    it "provides a link to the admin dashboard" do
      body_html = mail.body.parts.find { |p| p.content_type.match /html/ }.body.raw_source
      expect(body_html).to have_link('Go to admin dashboard', href: "http://#{ENV['SERVERNAME']}/status")
    end
  end
  ...
  describe "fatal error report" do
    ...
    it "provides a link to the admin dashboard" do
      body_html = mail.body.parts.find { |p| p.content_type.match /html/ }.body.raw_source
      expect(body_html).to have_link('Go to admin dashboard', href: "http://#{ENV['SERVERNAME']}/notifications?level=fatal")
    end
  end

  describe "stale agent report" do
    ...
    it "provides a link to the admin dashboard" do
      body_html = mail.body.parts.find { |p| p.content_type.match /html/ }.body.raw_source
      expect(body_html).to have_link('Go to admin dashboard', href: "http://#{ENV['SERVERNAME']}/notifications?class=SourceNotUpdatedError")
    end
  end
