...
describe Filter, :type => :model do
  ...
  context "class methods" do
    ...
    context "changes" do
      before do
        @change = FactoryGirl.create(:change, previous_count: 12)
      ...
      it "should call all active filters" do
        ...
        expect(response[:message]).to include("Resolved 1 change")
        expect(response[:review_messages].size).to eq(Filter.active.count)
        expect(response[:review_messages].first).to include("Found 1 decreasing event count error in 1 change")
      end
    end
    ...
    context "no unresolved changes" do
      before do
    ...
    context "unresolve" do
      ...
      it "should unresolve changes" do
  ...
  context "decreasing event count" do
    ...
    context "real decrease" do
      let(:change) { FactoryGirl.create(:change, previous_count: 12) }
      let(:options) { { id: change.id } }
      ...
      it "should raise errors" do
        ...
        expect(notification.level).to eq(2)
        expect(notification.source_id).to eq(1)
      end
    end

    context "decrease to zero" do
      let(:change) { FactoryGirl.create(:change, event_count: 0) }
      let(:options) { { id: change.id } }
    ...
    context "skipped" do
      let(:change) { FactoryGirl.create(:change, event_count: 0, skipped: true) }
      let(:options) { { id: change.id } }
    ...
    context "API errors" do
      let(:change) { FactoryGirl.create(:change, event_count: nil) }
      let(:options) { { id: change.id } }
  ...
  context "increasing event count" do
    ...
    context "real increase" do
      let(:change) { FactoryGirl.create(:change, event_count: 3600) }
      let(:options) { { id: change.id } }
    ...
    context "same day" do
      let(:change) { FactoryGirl.create(:change, event_count: 3600, update_interval: 1) }
      let(:options) { { id: change.id } }
  ...
  context "work not updated" do
    ...
    let(:days) { 42 }
    let(:change) { FactoryGirl.create(:change, event_count: nil, update_interval: days) }
    let(:options) { { id: change.id } }
  ...
  context "source not updated" do
    ...
    let!(:mendeley) { FactoryGirl.create(:source, name: "mendeley") }
