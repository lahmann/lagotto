...
describe Wordpress, :type => :model do
  ...
  context "parse_data" do
    it "should report that there are no events if the doi is missing" do
      ...
      expect(response).to eq(doi: work.doi, source: "wordpress", :events=>[], :events_by_day=>[], :events_by_month=>[], :events_url=>nil, :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>nil, :groups=>nil, :comments=>nil, :likes=>nil, :citations=>0, :total=>0})
    end
    ...
    it "should report if there are no events and event_count returned by the Wordpress API" do
      ...
      result = { 'data' => "null\n" }
      response = subject.parse_data(result, work)
      expect(response).to eq(doi: work.doi, source: "wordpress", :events=>[], :events_by_day=>[], :events_by_month=>[], :events_url=>"http://en.search.wordpress.com/?q=\"#{work.doi_escaped}\"&t=post", :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>nil, :groups=>nil, :comments=>nil, :likes=>nil, :citations=>0, :total=>0})
    end
