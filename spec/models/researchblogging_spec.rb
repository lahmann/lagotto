...
describe Researchblogging, :type => :model do
  ...
  context "parse_data" do
    ...
    it "should report if the doi is missing" do
      ...
      expect(subject.parse_data(result, work)).to eq(doi: work.doi, source: "researchblogging", events: [], :events_by_day=>[], :events_by_month=>[], events_url: nil, event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 })
    end
    ...
    it "should report if there are no events and event_count returned by the ResearchBlogging API" do
      ...
      expect(response).to eq(doi: work.doi, source: "researchblogging", events: [], :events_by_day=>[], :events_by_month=>[], event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 }, events_url: subject.get_events_url(work))
    end
