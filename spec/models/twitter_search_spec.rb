...
describe TwitterSearch, :type => :model do
  ...
  context "get_data" do
    ...
    it "should catch errors with the Twitter Search API" do
      ...
      expect(response).to eq(error: "the server responded with status 408 for https://api.twitter.com/1.1/search/tweets.json?count=100&include_entities=1&q=10.1371%252Fjournal.pone.0000001+OR+http%3A%2F%2Fwww.plosone.org%2Fwork%2Finfo%253Adoi%252F10.1371%252Fjournal.pmed.0000001&result_type=recent", :status=>408)
      expect(stub).to have_been_requested
  ...
  context "parse_data" do
    it "should report if there are no events and event_count returned by the Twitter Search API" do
      work = FactoryGirl.create(:work_with_tweets, :doi => "10.1371/journal.pone.0000000")
      body = File.read(fixture_path + 'twitter_search_nil.json', encoding: 'UTF-8')
      result = JSON.parse(body)
      expect(subject.parse_data(result, work)).to eq(doi: work.doi, source: "twitter_search", events: [], :events_by_day=>[], :events_by_month=>[], event_count: 0, events_url: "https://twitter.com/search?q=#{work.doi_escaped}", event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: 0, likes: nil, citations: nil, total: 0 })
    end
    ...
    it "should report if there are events and event_count returned by the Twitter Search API" do
      ...
      expect(response[:events_url]).to eq("https://twitter.com/search?q=#{work.doi_escaped}")
