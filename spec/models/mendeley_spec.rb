...
describe Mendeley, :type => :model do
  ...
  context "lookup access token" do
    ...
    it "should report that there are no events if access token can't be retrieved" do
      ...
      expect(subject.get_data(work, options = { :agent_id => subject.id })).to eq({})
      expect(stub).to have_been_requested
      expect(Notification.count).to eq(1)
      notification = Notification.first
      expect(notification.class_name).to eq("Net::HTTPUnauthorized")
      expect(notification.message).to eq("the server responded with status 401 for https://api-oauth2.mendeley.com/oauth/token")
  ...
  context "parse_data for metrics" do
    ...
    let(:null_response) { { doi: work.doi, source: "mendeley", :events=>{}, :events_by_day=>[], :events_by_month=>[], :events_url=>nil, :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>0, :groups=>0, :comments=>nil, :likes=>nil, :citations=>nil, :total=>0 } } }
