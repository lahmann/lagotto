...
describe F1000, type: :model, vcr: true do
  ...
  context "parse_data from the f1000 internal database" do
    it "should report if there are no events and event_count returned by f1000" do
      ...
      expect(response).to eq(:events=>[], :events_by_day=>[], :events_by_month=>[], :event_count=>0, :events_url=>nil, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>nil, :groups=>nil, :comments=>nil, :likes=>nil, :citations=>0, :total=>0}, extra: [])
    end
