...
describe Wikipedia, :type => :model do
  ...
  context "get_data" do
    ...
    it "should catch errors with the Wikipedia API" do
      ...
      expect(notification.class_name).to eq("Net::HTTPBadRequest")
  ...
  context "parse_data" do
    it "should report if the doi is missing" do
      work = FactoryGirl.build(:work, :doi => nil)
      result = {}
      expect(subject.parse_data(result, work)).to eq(doi: work.doi, source: "wikipedia", events: {}, :events_by_day=>[], :events_by_month=>[], events_url: nil, event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 })
    end

    it "should report if there are no events and event_count returned by the Wikipedia API" do
      result = { "en"=>0 }
      expect(subject.parse_data(result, work)).to eq(doi: work.doi, source: "wikipedia", events: {"en"=>0, "total"=>0}, :events_by_day=>[], :events_by_month=>[], events_url: "http://en.wikipedia.org/w/index.php?search=\"#{work.doi_escaped}\"", event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 })
    end
    ...
    it "should catch errors with the Wikipedia API" do
      ...
      result = { "en"=>nil }
      response = subject.parse_data(result, work)
      expect(response).to eq(doi: work.doi, source: "wikipedia", :events=>{"en"=>nil, "total"=>0}, :events_by_day=>[], :events_by_month=>[], :events_url=>"http://en.wikipedia.org/w/index.php?search=\"#{work.doi_escaped}\"", :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>nil, :groups=>nil, :comments=>nil, :likes=>nil, :citations=>0, :total=>0})
    end
  end
end
