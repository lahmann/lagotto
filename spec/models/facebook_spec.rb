...
describe Facebook, :type => :model do
  ...
  context "parse_data" do
    ...
    it "should report if the doi and canonical URL are missing" do
      ...
      expect(subject.parse_data(result, work)).to eq(doi: work.doi, source: "facebook", :events=>{}, :events_by_day=>[], :events_by_month=>[], :events_url=>nil, :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>0, :groups=>nil, :comments=>0, :likes=>0, :citations=>nil, :total=>0})
    end
