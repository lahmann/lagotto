...
describe Pmc, type: :model, vcr: true do
  ...
  context "parse_data" do
    it "should report that there are no events if the doi is missing" do
      ...
      expect(subject.parse_data(result, work)).to eq(:events=>[], :events_by_day=>[], :events_by_month=>[], :events_url=>nil, :event_count=>0, :event_metrics=>{:pdf=>0, :html=>0, :shares=>nil, :groups=>nil, :comments=>nil, :likes=>nil, :citations=>nil, :total=>0}, extra: [])
    end
