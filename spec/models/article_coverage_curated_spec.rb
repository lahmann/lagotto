...
describe ArticleCoverageCurated, :type => :model do
  ...
  context "get_data from the Article Coverage API" do
    it "should report if work doesn't exist in Article Coverage source" do
      ...
      stub = stub_request(:get, subject.get_query_url(work)).to_return(:body => {"error" => "Work not found"}.to_json, :status => 404)
      expect(subject.get_data(work)).to eq(error: "Work not found", status: 404)
  ...
  context "parse_data from the Article Coverage API" do
    it "should report if the doi is missing" do
      work = FactoryGirl.build(:work, :doi => nil)
      result = {}
      expect(subject.parse_data(result, work)).to eq(doi: work.doi, source: "article_coverage_curated", :events=>[], :events_by_day=>[], :events_by_month=>[], :events_url=>nil, :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>nil, :groups=>nil, :comments=>0, :likes=>nil, :citations=>nil, :total=>0})
    end

    it "should report if work doesn't exist in Article Coverage source" do
      result = { error: "Work not found", status: 404 }
      response = subject.parse_data(result, work)
      expect(response).to eq(doi: work.doi, source: "article_coverage_curated", :events=>[], :events_by_day=>[], :events_by_month=>[], :events_url=>nil, :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>nil, :groups=>nil, :comments=>0, :likes=>nil, :citations=>nil, :total=>0})
    end
