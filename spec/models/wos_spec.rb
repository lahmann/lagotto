...
describe Wos, :type => :model do
  ...
  context "parse_data" do
    it "should report that there are no events if the doi is missing" do
      ...
      expect(response).to eq(doi: work.doi, source: "wos", :events=>{}, :events_by_day=>[], :events_by_month=>[], :events_url=>nil, :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>nil, :groups=>nil, :comments=>nil, :likes=>nil, :citations=>0, :total=>0})
    end
    ...
    it "should report if there are no events and event_count returned by the Wos API" do
      ...
      result.extend Hashie::Extensions::DeepFetch
      response = subject.parse_data(result, work)
      expect(response).to eq(doi: work.doi, source: "wos", :events => {}, :events_by_day=>[], :events_by_month=>[], :event_count => 0, :events_url => nil, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 })
    end
